 ...
 * \ Exception Class
 *
 * A helper class used to send proper exceptions when specific memory allocation or boundry issues occur.
 */
class OAException
{
  public:
    ...
    /*! \fn OAException(OA_EXCEPTION ErrCode, const std::string& Message)
     *  \brief ErrCode The error code in the exception.
     *  \param Message The output message for the client.
     *  \param ErrCode The error code in the exception.
     *  \return an OAException object.
     */
...
class ObjectAllocator
{
  ...
  private:
    ...
    unsigned int get_page_size();
    void release_memory();
    bool is_on_freelist(void *Object) const;
    bool is_out_of_bounds(void *Object) const;
    void* awesome_cool_MyOAalloc(const char *label = 0);
    void* lame_old_newdelAlloc();
