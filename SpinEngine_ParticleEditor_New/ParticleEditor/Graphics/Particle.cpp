...
void Particle::run_particle(float dt, float p_lifetime, float lifetime_invariant, Vector3 pos, float ang_1,
...
void Particle::reset_particle(Vector3 pos, float p_lifetime, float ang_1, float ang_2, float spd1,
  float spd2, float acc1, float acc2, float spin)
{
  ...
  float angle_new = random_number(min(ang_1, ang_2), max(ang_1, ang_2));
  float speed = random_number(min(spd1, spd2), max(spd1, spd2));
  float acc = random_number(min(acc1, acc2), max(acc1, acc2));
...
void Particle::run_particle_square(float dt, float p_lifetime, float lifetime_invariant, Vector3 pos, float width,
...
void Particle::reset_particle_square(Vector3 pos, float p_lifetime, float width /*= 1.0f*/, float height /*= 1.0f*/,
...
void Particle::run_particle_circle(float dt, float p_lifetime, float lifetime_invariant,  Vector3 pos, float radius /*= 1.0f*/,
...
void Particle::reset_particle_circle(Vector3 pos, float p_lifetime, float radius, float spd1 /*= 1.0f*/,
...
void Particle::run_particle_diamond(float dt, float p_lifetime, Vector3 pos, float width /*= 1.0f*/, 
...
void Particle::reset_particle_diamond(Vector3 pos, float width /*= 1.0f*/, float height /*= 1.0f*/, 
