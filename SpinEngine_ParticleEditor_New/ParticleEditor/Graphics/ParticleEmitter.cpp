...
ParticleEmitter::ParticleEmitter(GameObject Owner, IDirect3DDevice9* device, emitter_types type, blend_types b_type,
  unsigned int particle_count, std::string t_name, D3DXCOLOR color, D3DXCOLOR  end_color) : device(device),
  ...
  //BLEND FACTORS
  use_source_factor(true),
  source_factor(BFT_SRC_COLOR),
  use_dest_factor(true),
...
bool ParticleEmitter::Initialize()
{
  ...
  Vector3 initPos = Owner->GetTransform()->GetPosition();
...
void ParticleEmitter::Render(IDirect3DDevice9* device)
{
  ...
  d3ddev->SetRenderState(D3DRS_ALPHAREF, 8);
  d3ddev->SetRenderState(D3DRS_BLENDOP, D3DBLENDOP_ADD);    // set the operati
